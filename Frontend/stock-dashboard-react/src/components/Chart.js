  ...
  const formatData = (liveData) => {
    return liveData.map((item, index) => {
      ...
      return {
        ...
        close: parseFloat(item.low).toFixed(2),
        close: parseFloat(item.close).toFixed(2),
        adjClose: parseFloat(item.adjClose).toFixed(2),
        volume: parseFloat(item.volume).toFixed(2),
        date: filter == "1D" ? getTimeFromTimestamp(item.date): getDateFromTimestamp(item.date)
      };
    });
  };
